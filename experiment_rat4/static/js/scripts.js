...
class Game {
    ...
    move(x, y, record = true) {
       ...
       // }
        ...
        if (this.current === 27) {
            this.end();
            window.location.href = "/" + currentGeneration + "/PipedText";
        }
    ...
    finishMove() {
        ...
        $.post("/api/move/end", {
            ...
            setTimeout(() => {
                location.href = "/PipedText"
            }, 5000)
        })

        this.grid.grid.animate({ opacity: .5 }, "slow")
    }
...
class Tutorial {
    constructor (grid_element, boards, colors, callback = null, time_between_moves = 1500, time_between_games = 3000) {
        this.game = 0
        this.x = 0
        this.y = 0
        this.current = 0
    ...
    clearSteppedImageContainer() {
        $("#stepped-images-container").html('<strong>Plants Gathered:</strong>');
    }
