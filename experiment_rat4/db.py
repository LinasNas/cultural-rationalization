...
class Player(Base):
    ...
    #NEW:Linas
    Ignorance_Knowledge = CharField(max_length=16, default=None)
    ...
    Conformity_Importance = IntegerField(default=None)  # Store 1-7 Likert scale response
    textContinuityImportance = CharField(max_length=1024, default=None)

    valueChoice = CharField(max_length=15, default=None)
    nameChoice = CharField(max_length=15, default=None)
    tutorialChoice = CharField(max_length=15, default=None)
    endFreeText = CharField(max_length=1024, default=None)
    PipedText = TextField(default=None)
...
def update_keep_column(player):
    ...
    if (player.total_time <= 900 and player.total_time > 0 and player.tab_switch_count <= 1 and player.direction_time >= 25 and player.attention_check == "Ferngrove" and player.worker_id is not None and (player.tutorialChoice == "I" or player.tutorialChoice is None)):
        player.keep = 1
        print(f"Setting keep to 1 for player with ID: {player.id}")
    else:
        player.keep = 0
