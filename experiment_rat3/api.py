...
def consent_player(generation_ID):
    ...
    NUM_MICROCULTURES = 1
    PLAYERS_PER_MICROCULTURE = 4000000000000
...
def current_player_tutorial(generation_ID):
    ...
    if pc_assigned:
        ...
        #OLD
        # previous_players = Player.select().where(Player.total_time > 0, Player.id != player.id)

        #NEW: V1
        # previous_players = Player.select().where(Player.total_time > 0, Player.id != player.id, Player.keep == 1, Player.Generation == prev_gen_ID)
        ...
        previous_players = Player.select().where(Player.total_time > 0, Player.id != player.id, Player.keep == 1, Player.Generation == prev_gen_ID, Player.Microculture == player.Microculture)
        ...
        #IS THIS WHERE WE FILTER WHETHER WE SHOW THE TUTORIAL? If so: change:
        #OLD
        #if len(players) >= GENERATION_SIZE:
        #NEW
        if (len(players) >= 9999999999999999999) & (is_gen_0 == False):

            #this will sort out all people from previous generation only
            players = sorted(players, key=lambda x: x.created)

            #CHANGE THIS: to all the players from the last gen
            #If no last gen, then no tutorials
            #gen 1: players[0:15] OR [0:18]
            #also: only choose players who have 1 in the keep col


            #for this, only use players from (1) past gen; (2) filtered by keep
            print("players", players)
            print('len(players)', len(players))

            #OLD
            #gen = players[86:98]
            #NEW: these are already filtered by keep
            gen = players
            print("gen", gen)
            print('len(gen)', len(gen))

            gen_weights = [p.total_score for p in gen]
            #gen_weights = softmax([p.total_score for p in gen])
            print(gen_weights, gen)
            gen_players = weighted_sample_without_replacement(gen, gen_weights, k=max_tuts)
            for gen_player in gen_players:
                players_chosen.append(gen_player)
        elif manual_tutorial== 1:
            specific_players = [2003,1962,2162]#2003,1962,2162
...
def save_grind_guesses():
    ...
    keys = ["optionGrindSQ0", "optionGrindSQ1", "optionGrindSQ2", "optionGrindSQ3"]
    for key in keys:
        if not request.form.get(key):
            return api_error(f"Missing argument for guesses {key}", status=400)

    player.grinding_guess_0 = request.form.get("optionGrindSQ0")
    player.grinding_guess_1 = request.form.get("optionGrindSQ1")
    player.grinding_guess_2 = request.form.get("optionGrindSQ2")
    player.grinding_guess_3 = request.form.get("optionGrindSQ3")
    player.save()
