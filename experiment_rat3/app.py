...
def get_collected_items(player_id):
    try:
        ...
        # Retrieve color mappings for both original and current player
        original_player_colors = original_player.color_combination.to_dict()["colors"]
        current_player_colors = current_player.color_combination.to_dict()["colors"]

        # Debug log to verify color mappings
        print(f"Original player colors: {original_player_colors}")
        print(f"Current player colors: {current_player_colors}")

        # Determine the latest collection times for the special items (color "5" and "6")
        special_item_5_collected_at = None
        special_item_6_collected_at = None
        original_color_id_5 = None
        original_color_id_6 = None
        for item in collected_items:
            if item.color == "5" and special_item_5_collected_at is None:
                special_item_5_collected_at = item.collected_at
                original_color_id_5 = next((color_id for color_id, color in original_player_colors.items() if color == item.color), None)
            if item.color == "6" and special_item_6_collected_at is None:
                special_item_6_collected_at = item.collected_at
                original_color_id_6 = next((color_id for color_id, color in original_player_colors.items() if color == item.color), None)
        ...
        for item in collected_items:
            ...
            mapped_color = current_player_colors.get(original_color_id)
            if mapped_color:
                is_marked_by_5 = special_item_5_collected_at is not None and item.collected_at <= special_item_5_collected_at
                is_marked_by_6 = special_item_6_collected_at is not None and item.collected_at <= special_item_6_collected_at
                item_data = {
                    "color": mapped_color,
                    "original_color": original_color_id,
                    "collected_at": item.collected_at.strftime("%Y-%m-%d %H:%M:%S"),
                    "marked_by_5": is_marked_by_5,
                    "marked_by_6": is_marked_by_6
                }
                items.append(item_data)

        # Map original colors for 5 and 6 to current player's colors
        mapped_color_5 = current_player_colors.get(original_color_id_5)
        mapped_color_6 = current_player_colors.get(original_color_id_6)

        response = {
            "items": items,
            "mapped_color_5": mapped_color_5,
            "mapped_color_6": mapped_color_6
        }

        # Debug log to verify final items list
        print(f"Collected items for player {player_id}: {response}")
